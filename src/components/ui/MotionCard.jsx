...
const getThemeDefaults = (theme, variant, intensity, rounded, glow, highlight, effect) => {
...
const getIntensityFactor = (intensity) => {
  switch (intensity) {
...
const getHighlightIntensity = (highlight) => {
  switch (highlight) {
...
const MotionCard = ({
  ...
  useEffect(() => {
    ...
    if (glowRef.current && !disableGlow && glowIntensity > 0) {
      ...
      glow.style.opacity = isHovered ? glowIntensity : 0;
    }
...
const MotionCardContent = ({ className = '', children, ...props }) => {
...
export { MotionCard, MotionCardContent, MotionCardLayer };