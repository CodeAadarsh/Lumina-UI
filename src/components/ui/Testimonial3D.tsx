...
// Define the variant props types using the CVA utility
...
export interface TestimonialProps extends 
  React.HTMLAttributes<HTMLDivElement>,
  Omit<TestimonialVariantProps, 'theme'> {
 ...
 */
const Testimonial3D: React.FC<TestimonialProps> = ({
  children,
  className,
  theme: themeProp,
  size,
  ...
  const { theme: contextTheme } = useSafeTheme() as ThemeContextType;
  ...
  const handle3DEffect = (e: React.MouseEvent<HTMLDivElement>): void => {
  ...
  const getAvatarAnimationClass = (): string => {
  ...
  const getEntryAnimationClass = (): string => {
    ...
    return entranceAnimations[animation as string] || '';
  };
...
// For easier usage
const Testimonial: React.FC<TestimonialProps> = Testimonial3D;
